...
def get_contract_fun(weight, implementation="reconstructed", separable=False):
    ...
    if implementation == "reconstructed":
        ...
        raise ValueError(f'Got implementation={implementation}, expected "reconstructed" or "factorized"')
...
class SpectralConv(BaseSpectralConv):
    ...
    def __init__(
        ...
        self.resolution_scaling_factor: Union[None, List[List[float]]] = validate_scaling_factor(resolution_scaling_factor, self.order)
        ...
        if separable:
            if in_channels != out_channels:
                raise ValueError(
                    "To use separable Fourier Conv, in_channels must be equal " f"to out_channels, but got in_channels={in_channels} and " f"out_channels={out_channels}",
                )
        ...
        if factorization is None:
            ...
            self.weight = FactorizedTensor.new(weight_shape, rank=self.rank, factorization=factorization, fixed_rank_modes=fixed_rank_modes, **tensor_kwargs, dtype=torch.cfloat)
...
class SpectralConv1d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        out_fft[slices] = self._contract(x[slices], self.weight[slices], separable=self.separable)
...
class SpectralConv2d(SpectralConv):
    ...
    def forward(self, x):
        ...
        out_fft[slices0] = self._contract(x[slices0], self.weight[slices1], separable=self.separable)

        """Lower block"""
        out_fft[slices1] = self._contract(x[slices1], self.weight[slices0], separable=self.separable)
...
class SpectralConv3d(SpectralConv):
    ...
    def forward(self, x):
        ...
        """Upper block -- truncate high frequencies."""
        out_fft[slices0] = self._contract(x[slices0], self.weight[slices3], separable=self.separable)

        """Low-pass filter for indices 2 & 4, and high-pass filter for index 3."""
        out_fft[slices1] = self._contract(x[slices1], self.weight[slices2], separable=self.separable)

        """Low-pass filter for indices 3 & 4, and high-pass filter for index 2."""
        out_fft[slices2] = self._contract(x[slices2], self.weight[slices1], separable=self.separable)

        """Lower block -- low-cut filter in indices 2 & 3
        and high-cut filter in index 4."""
        out_fft[slices3] = self._contract(x[slices3], self.weight[slices0], separable=self.separable)
